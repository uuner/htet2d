...
case_box_on_bottom = assert (isValid field)
  where field = (emptyField FigO N FigL N) {
    getCurrentFigure = FigO, 
    getCurrentPosition = Position (Coord 2 (fieldHeight - 2)) N,
...
case_box_just_fell = (expected, 10 + 3, 123 + 300) @=? (getDebris tested, getLines tested, getScores tested)
  where 
  tested = fromLeft $ nextStep field
  field = (emptyField FigO N FigO N) {
    getDebris = createDebris ((Coord 4 6):(Coord 3 3):(Coord 1 1):(Coord 1 0)
        :[Coord x y | x <- [0..fieldWidth - 1], y <- [2,4,5]]),
    getCurrentFigure = FigO, 
    getCurrentPosition = Position (Coord 0 (fieldHeight - 1 - 7)) N,
    getGameState = InProgress,
    getScores = 123,
    getLines = 10
  }
